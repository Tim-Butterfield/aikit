...
The recommended high-value core to wrap is `doctor --require-root`, `batch start`,
...
auto-selected. For commands that prepare or snapshot state (`init --require-repo`,
...
- **Keep broad scans, broad review bundles, and patch output confirmation-gated.**
  ...
  repo`, and `batch diff --patch` can surface more content than intended.
...
- **Repo readiness:** `doctor --require-root`, with guarded `init --require-repo`.
- **Task anchoring:** `batch start`, `batch changed`, with optional `batch list` /
  `batch show` as helpers.
- **Change inspection:** `batch diff`, stat-oriented by default; patch output
  confirmation-gated.
...
- **Basis commands:** `aikit doctor --json`; guarded `aikit init --json`.
- **Inputs:** none beyond the current working directory.
- **Output to preserve:** `repo_root`, `root_source`, `git_branch`, `git_head`,
  `tracked_tree_clean`, `ready`, `warnings`, the `.aikit/temp/` existence flag,
  and (after `init`) what was created vs. already present.
- **Blocked states to surface:** none from `aikit doctor` — it reports rather than
  blocking, which is why it is the right probe to lead with. (`aikit doctor --require-root`
  blocks `blocked_repo_not_found` outside a marker; use it only when you *want*
  the absence of a root to be an error.)
- **Safety boundary:** `doctor` is read-only in every path, including when it
  reports that nothing is initialized. `init` is idempotent but mutates local VCS
  metadata (`.git/info/exclude`) and creates `.aikit/temp/`; it never edits
  `.gitignore` and never touches remotes.
- **Can run automatically?** `doctor` yes. `init` should be guarded — run it only
  when `doctor` reports a readiness gap, and prefer to surface that it will write
  local VCS metadata. Check `root_source` before treating `repo_root` as an
  anchor: `cwd_no_marker` means there is no root, only the directory you were in.
...
- **Basis commands:** `aikit batch changed --anchor <anchor> --json` (optionally
  `--hash`); `aikit batch diff <anchor> --json` (stat-oriented by default).
...
- **Output to preserve:** the file list, per-source counts, sizes, hashes (when
  `--hash`), and for `batch diff` the base/current heads, name-status list, and
  stat.
- **Blocked states to surface:** `blocked_missing_anchor`,
  `blocked_invalid_anchor`, cross-repo rejection, and (for `batch diff`) a
...
- **Can run automatically?** Yes, with an explicit anchor and stat-oriented diff
  output.
- **Explicit confirmation required?** Stat output, no. `batch diff --patch`
...
- **Problem solved:** a cheap redacted sweep for likely committed secrets before
...
- **Blocked states to surface:** `blocked_path_escape` for out-of-repo/symlink
  paths; `blocked_secret_findings` (exit 3) when `--fail-on <severity>` is used
  and a finding at that severity or above exists.
- **Safety boundary:** read-only, creates no artifacts, never prints raw values.
  Severity is confidence that the match is a real credential, not blast radius. A
  finding is not proof of a live credential, and clean output is not proof a file
  is safe to share — the rule set favours precision over coverage and does not
  replace gitleaks or trufflehog.
...
- `batch diff --patch` — full patch text can be large or reveal sensitive
  content.
