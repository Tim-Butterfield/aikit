...
Expected current repository: a clone of this repository, at whatever path it lives on the
machine running the work.
...
- The `repo` command family prepares and inspects repo-local aikit setup, using the same
  ...
  - `aikit init --require-repo` — prepare the current repository for local aikit usage. Detects the
  ...
  - `aikit doctor --require-root` — report repo-local readiness without mutating anything
...
- Extend the `batch` family and add a `diff` family, all mechanical inspection/diff
  ...
  - `aikit batch diff <anchor>` — diff the anchor's recorded `git_head`
...
- Add an `env` family and a `scan` family — mechanical, local commands that call no AI
  ...
  - `aikit scan secrets <path>...` — a best-effort heuristic secret scan over explicit
    ...
    binary files and files over `--max-file-bytes` (default 1 MiB). Rule set = format rules
    for self-identifying credential formats plus one name-based rule; severity is confidence
    the match is a real credential (`high`/`medium`/`low`), not blast radius. NEVER prints raw
    secret values (human or JSON). Default exit 0; `--fail-on <high|medium|low>` exits 3 with
    `blocked_secret_findings` at that severity or above. Creates no artifacts. `--json`.
...
- **Slice 1 (implemented):** `aikit script run`, `aikit script check`; remove
  `aikit run script`.
- **Slice 2 (implemented):** `aikit init --require-repo`, `aikit doctor --require-root`.
- **Slice 3 (implemented):** `aikit output list`, `aikit output show`,
  `aikit output clean`.
- **Slice 4 (implemented):** `aikit batch list`, `aikit batch show`, `aikit batch diff`.
