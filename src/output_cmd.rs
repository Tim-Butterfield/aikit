...
use crate::output::resolve_output_root;
use crate::repo;
...
pub fn show(args: OutputShowArgs) -> Result<(), AikitError> {
    ...
    let repo_canon = crate::formats::canonicalize(&root)
        .map_err(|e| AikitError::other(format!("failed to resolve repo root: {e}")))?;
    let output_root = resolve_output_root(&repo_canon, args.root.as_deref())?;
    let output_root_canon =
        crate::formats::canonicalize(&output_root).unwrap_or(output_root.clone());
    ...
    for base in [&repo_canon, &output_root] {
        ...
        if cand.exists() {
            if let Ok(c) = crate::formats::canonicalize(&cand) {
