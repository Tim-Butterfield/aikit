...
pub fn start(args: StartArgs) -> Result<(), AikitError> {
    ...
    // Atomic: an anchor is read back later by `batch changed` and `diff anchor`, and a
    // half-written one would fail as `blocked_invalid_anchor` — a message that blames the
    // anchor rather than the interrupted write that produced it.
    crate::formats::write_with_newline(&file_path, &body).map_err(|e| {
        AikitError::other(format!(
...
pub fn changed(args: ChangedArgs) -> Result<(), AikitError> {
    let root = repo::detect_root()?;
    // An anchor id resolves the same way here as it does for `batch diff`. Requiring a path
    // in one place and accepting an id in the other made the storage layout part of the
    // interface for half the commands: a caller who moved the output root, or who only ever
    // saw the id printed by `batch start`, could use one and not the other.
    ...
    let anchor_path = resolve_anchor_path(&repo_canon, &default_root, &args.anchor)?;
    let anchor_ref = anchor_path.to_string_lossy().into_owned();

    let anchor = load_anchor(&anchor_ref, &root)?;
    let reference = anchor_reference_time(&anchor_ref, &anchor);
...
fn walk_changed_since_anchor(
    ...
    let root_canon = crate::formats::canonicalize(repo_root)
        .map_err(|e| AikitError::other(format!("failed to resolve repo root: {e}")))?;
...
fn safe_batches_dir(output_root: &Path) -> Option<PathBuf> {
    ...
    match fs::symlink_metadata(&batches) {
        Ok(m) if m.is_dir() && !m.file_type().is_symlink() => {
            crate::formats::canonicalize(&batches).ok()
        }
        _ => None,
    }
}
...
pub fn resolve_anchor_path(
    ...
    if !looks_like_path {
        ...
        if let Ok(m) = fs::symlink_metadata(&cand) {
            if m.is_file() && !m.file_type().is_symlink() {
                if let Ok(real) = crate::formats::canonicalize(&cand) {
    ...
    let real = crate::formats::canonicalize(&raw).map_err(|_| {
        AikitError::blocked(
            blocked::MISSING_ANCHOR,
...
fn same_repo(repo_root: &Path, anchor_root: &str) -> bool {
    match (
        crate::formats::canonicalize(repo_root).ok(),
        crate::formats::canonicalize(anchor_root).ok(),
    ) {
        (Some(a), Some(b)) => a == b,
