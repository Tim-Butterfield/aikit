//! `aikit batch diff <anchor>` — a mechanical Git diff report from a batch anchor's
//! recorded head to the current working-tree state.
//!
//! The module keeps its own name because the diff machinery is distinct from anchor
//! creation; only the command surface moved under `batch`, where its subject lives.
//!
...
/// `aikit batch diff` — diff the anchor's recorded `git_head` against the current
/// working tree.
