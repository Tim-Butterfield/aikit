...
use crate::cli::{InitArgs, RepoDoctorArgs};
...
`aikit init --require-repo` inside a repository) to create .aikit/",
        )
    })
}
        ...
        if content.is_empty() {
            content.push_str(
                "# Local-only ignore patterns for aikit working files.\n\
                 # Managed by `aikit init --require-repo`; registered via .hg/hgrc and never committed.\n",
            );
...
enum InitKind {
    ...
    Auto,
    /// `aikit init --require-repo` — force repo mode; error if not inside a repo.
    Repo,
    /// `aikit init --require-folder` — force non-repo mode; error if inside a repo.
    Folder,
}
...
impl InitKind {
    fn command_label(self) -> &'static str {
        match self {
            ...
            InitKind::Repo => "aikit init --require-repo",
            InitKind::Folder => "aikit init --require-folder",
        }
    }
}
...
///
/// `--require-repo` / `--require-folder` turn the adaptive behaviour into an assertion.
/// The flags are mutually exclusive (clap enforces it), so the mapping is total.
pub fn init(args: InitArgs) -> Result<(), AikitError> {
    let kind = match (args.require_repo, args.require_folder) {
        (true, _) => InitKind::Repo,
        (_, true) => InitKind::Folder,
        _ => InitKind::Auto,
    };
    do_init(kind, args.json)
}
    ...
    let (root, vcs): (PathBuf, Option<Vcs>) = match kind {
        InitKind::Repo => match vcs_root {
            ...
            None => {
                return Err(AikitError::blocked(
                    blocked::REPO_NOT_FOUND,
                    "`aikit init --require-repo` requires a Git or Mercurial repository; run \
`aikit init --require-folder` (or `aikit init`) to initialize a non-repo folder",
                ))
            }
        },
        InitKind::Folder => match vcs_root {
            Some((_, v)) => {
                return Err(AikitError::blocked(
                    ...
                    format!(
                        "this directory is inside a {} repository; run `aikit init --require-repo` \
...
/// Which doctor variant is being run, controlling marker-detection policy only.
///
/// Note this axis is strict-vs-adaptive, **not** repo-vs-folder as with [`InitKind`].
/// Init splits three ways because it *mutates*: the mutation differs per variant and
/// `folder init` must refuse inside a repo. Doctor writes nothing, so there is no mutation
/// to vary and nothing to refuse — a `folder doctor` would have no distinct body and no
/// safety rationale.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum DoctorKind {
    /// `aikit doctor` — adaptive: report against the nearest marker, or against the current
    /// directory as not-ready when there is no marker at all.
    Auto,
    /// `aikit doctor --require-root` — require a marker; block when there is none.
    Repo,
}

/// `aikit doctor` — readiness report. Read-only: creates and modifies nothing (no
/// `.aikit/`, `.scratch/`, `.claude/`, `.gitignore`, or `.git/info/exclude`), including on
/// the no-marker path.
///
/// Adaptive by default: with no `.git`, `.hg`, or `.aikit` anywhere up the tree it reports
/// `ready: false` against the current directory at **exit 0** rather than blocking, so the
/// documented "doctor first, then init only if doctor says so" loop works in a fresh folder
/// — where a read-only probe failing is precisely the wrong answer.
///
/// `--require-root` turns that into an assertion, blocking `blocked_repo_not_found`. Note
/// it requires a *marker*, not a repository: a non-repo `.aikit/` folder satisfies it.
pub fn doctor(args: RepoDoctorArgs) -> Result<(), AikitError> {
    let kind = if args.require_root {
        DoctorKind::Repo
    } else {
        DoctorKind::Auto
    };
...
/// Shared implementation behind `doctor` / `repo doctor`. `kind` governs root resolution
/// only; everything after it is identical, which is what keeps the two reports the same
/// record rather than two shapes a caller must distinguish.
fn do_doctor(kind: DoctorKind, args: RepoDoctorArgs) -> Result<(), AikitError> {
    // Filesystem detection: a Git/Mercurial repo, or a non-repo `.aikit/` folder.
    let (root, vcs, root_source) = match detect_marker_root() {
        Ok((root, vcs)) => (root, vcs, "marker"),
        Err(e) => match kind {
            DoctorKind::Repo => return Err(e),
            DoctorKind::Auto => {
                let cwd = std::env::current_dir().map_err(|e| {
                    AikitError::other(format!("failed to resolve current directory: {e}"))
                })?;
                (cwd, None, "cwd_no_marker")
            }
        },
    };
    let aikit = root.join(".aikit");
...
state not determined"
    ...
    // With no marker there is nothing to be missing *from* — the folder simply has not been
    // initialized. Say that instead of listing every absent directory as its own defect.
    if root_source == "cwd_no_marker" {
        warnings.push(
            "no aikit root found from the current directory: run `aikit init` to create \
.aikit/ (add `--require-repo` to refuse unless this is a repository)"
                .to_string(),
        );
    } else if !temp_dir_exists {
    ...
    if vcs.is_some() && !aikit_ignored {
        warnings.push(
            ".aikit/ is not ignored; run `aikit init --require-repo` to add local ignore coverage"
                .to_string(),
        );
    }
...
sh, bash, zsh, pwsh, powershell, cmd, python3, python, node)"
    ...
    // An unanchored report is never ready: there is no root, so there is nothing to be
    // ready. This is independent of the checks above, which would otherwise let a folder
    // that happens to contain a `temp/` look initialized.
    let ignore_ok = vcs.is_none() || aikit_ignored;
    let ready = root_source == "marker" && temp_dir_exists && ignore_ok && any_runner_available;
    ...
    if args.json {
        ...
        let label = match kind {
            DoctorKind::Auto => "aikit doctor",
            DoctorKind::Repo => "aikit doctor --require-root",
        };
        println!("{label} (read-only):");
        if record.root_source == "cwd_no_marker" {
            // Never print a bare path here: cwd is where the report was taken, not a root,
            // and the two are indistinguishable once written as a path on its own.
            println!(
                "  no aikit root found (no .git, .hg, or .aikit up the tree)\n  \
                 reporting against the current directory: {}",
                record.repo_root
            );
        } else {
        ...
        for r in &record.runners {
            let state = match r.reason.as_str() {
                "available" => "available",
                "available_via_alias" => "available (app execution alias)",
                "not_found_on_path" => "not found",
                _ => "not applicable (other OS)",
            };
            // Path and version are what make an "available" line actionable: they are how a
            // reader tells powershell 5.1 from pwsh 7 without running anything themselves.
            match (&r.path, &r.version) {
                (Some(p), Some(v)) => println!("    {} ({state}) {v} — {p}", r.name),
                (Some(p), None) => println!("    {} ({state}) — {p}", r.name),
                _ => println!("    {} ({state})", r.name),
            }
        }
