...
pub struct ResolvedConfig {
    ...
    /// How many run directories under `.aikit/outputs/runs/` to keep, pruned oldest-first
    /// after each run. `0` disables pruning.
    ///
    /// On by default because per-invocation directories accumulate at agent speed and
    /// nothing else removes them — `output clean` is a command a human has to remember. Only
    /// `runs/` is pruned; batches, reviews and inventories are created deliberately.
    pub retain_runs: usize,
    /// Repo-relative paths of the config files that were loaded, in precedence order.
    pub sources: Vec<String>,
    /// Where each effective value came from: setting key → `"default"` or the repo-relative
    /// config file that last set it.
    ///
    /// Layered config is only explainable if a reader can see which layer won. Without this,
    /// answering "why is this value what it is?" means opening every candidate file in
    /// precedence order and replaying the merge by hand — and the CLI-consults-config /
    /// MCP-does-not split makes that worse, because the same file explains one surface and
    /// not the other.
    pub value_sources: BTreeMap<String, String>,
}

/// Default number of run directories kept. Generous enough that a normal session's audit
/// trail survives, small enough that the directory does not become the problem.
pub const DEFAULT_RETAIN_RUNS: usize = 100;
...
/// Every setting `config show` reports, in a stable display order.
///
/// Listed explicitly rather than derived, so a field added to `ResolvedConfig` without being
/// reported here is caught by the test that compares the two.
pub const SETTING_KEYS: &[&str] = &[
    "bundle.single_file",
    "bundle.embed_manifest",
    "bundle.sidecar_manifest",
    "bundle.output",
    "discovery.include_ignored_batch_files",
    "discovery.include_globs",
    "discovery.exclude_globs",
    "discovery.include_files",
    "script_runner.preferred_runners",
    "script_runner.detect_from_shebang",
    "script_runner.detect_from_extension",
    "script_runner.extension_map",
    "output.retain_runs",
];

impl ResolvedConfig {
    /// The effective value of every setting, as JSON, paired with where it came from.
    pub fn effective(&self) -> Vec<(&'static str, serde_json::Value, String)> {
        use serde_json::json;
        let v = |k: &str, value: serde_json::Value| {
            let source = self
                .value_sources
                .get(k)
                .cloned()
        ...
        SETTING_KEYS
            ...
            .map(|&k| {
                let value = match k {
                    "bundle.single_file" => json!(self.single_file),
                    "bundle.embed_manifest" => json!(self.embed_manifest),
                    "bundle.sidecar_manifest" => json!(self.sidecar_manifest),
                    "bundle.output" => json!(self.output),
                    "discovery.include_ignored_batch_files" => {
                        json!(self.include_ignored_batch_files)
                    }
                    "discovery.include_globs" => json!(self.include_globs),
                    "discovery.exclude_globs" => json!(self.user_exclude_globs),
                    "discovery.include_files" => json!(self.include_files),
                    "script_runner.preferred_runners" => {
                        json!(self.script_runner.preferred_runners)
                    }
                    "script_runner.detect_from_shebang" => {
                        json!(self.script_runner.detect_from_shebang)
                    }
                    "script_runner.detect_from_extension" => {
                        json!(self.script_runner.detect_from_extension)
                    }
                    "script_runner.extension_map" => json!(self.script_runner.extension_map),
                    "output.retain_runs" => json!(self.retain_runs),
                    other => unreachable!("unreported setting key: {other}"),
                };
                let (value, source) = v(k, value);
                (k, value, source)
            })
            .collect()
    }
...
impl ResolvedConfig {
    ...
    /// Merge one config file's values, recording `source` for each value it actually set.
    ///
    /// Provenance is recorded per assignment rather than per file, because a later file that
    /// sets one key does not make it the source of the others.
    fn merge_raw(&mut self, raw: RawConfig, source: &str) {
        macro_rules! set {
        ...
        if let Some(bundle) = raw.bundle {
            if let Some(v) = bundle.single_file {
                set!("bundle.single_file", self.single_file, v);
            }
            if let Some(v) = bundle.embed_manifest {
                set!("bundle.embed_manifest", self.embed_manifest, v);
            }
            if let Some(v) = bundle.sidecar_manifest {
                set!("bundle.sidecar_manifest", self.sidecar_manifest, v);
            }
            if let Some(v) = bundle.output {
                set!("bundle.output", self.output, Some(v));
            }
        }
        if let Some(d) = raw.discovery {
            if let Some(v) = d.include_ignored_batch_files {
                set!(
                    "discovery.include_ignored_batch_files",
                    self.include_ignored_batch_files,
                    v
                );
            }
            if let Some(v) = d.include_globs {
                self.value_sources
                    .insert("discovery.include_globs".to_string(), source.to_string());
                self.include_globs = v;
            }
            if let Some(v) = d.exclude_globs {
                set!("discovery.exclude_globs", self.user_exclude_globs, v);
            }
            if let Some(v) = d.include_files {
                set!("discovery.include_files", self.include_files, v);
            }
        }
        if let Some(s) = raw.script_runner {
            if let Some(v) = s.preferred_runners {
                set!(
                    "script_runner.preferred_runners",
                    self.script_runner.preferred_runners,
                    v
                );
            }
            if let Some(v) = s.detect_from_shebang {
                set!(
                    "script_runner.detect_from_shebang",
                    self.script_runner.detect_from_shebang,
                    v
                );
            }
            if let Some(v) = s.detect_from_extension {
                set!(
                    "script_runner.detect_from_extension",
                    self.script_runner.detect_from_extension,
                    v
                );
            }
            if let Some(map) = s.extension_map {
                ...
                set!(
                    "script_runner.extension_map",
                    self.script_runner.extension_map,
                    normalized
                );
            }
        }
        if let Some(o) = raw.output {
            if let Some(v) = o.retain_runs {
                set!("output.retain_runs", self.retain_runs, v);
            }
        }
    }
}
...
pub fn load(repo_root: &Path) -> Result<ResolvedConfig, AikitError> {
    ...
    for rel in ["aikit.config.json", ".aikit/config.json"] {
        ...
        cfg.merge_raw(raw, rel);
        cfg.sources.push(rel.to_string());
    }
    Ok(cfg)
}

/// `aikit config show` — report the fully resolved configuration and where each value came
/// from.
///
/// Read-only. Loading is the same code path every command uses, so what this prints is what
/// they will act on — a reimplementation here could be wrong in exactly the situation the
/// command exists to diagnose.
pub fn show(args: crate::cli::ConfigShowArgs) -> Result<(), AikitError> {
    let (root, _vcs) = crate::repo::detect_marker_root()?;
    let cfg = load(&root)?;

    let settings: Vec<serde_json::Value> = cfg
        .effective()
        .into_iter()
    ...
    if args.json {
        let record = serde_json::json!({
            ...
            "kind": crate::formats::KIND_CONFIG_SHOW,
            "repo_root": root.display().to_string(),
            // Files that exist and were layered, in precedence order (later wins).
            "sources": cfg.sources,
            "settings": settings,
        });
        println!(
            ...
            serde_json::to_string_pretty(&record)
                .map_err(|e| AikitError::other(format!("failed to serialize config: {e}")))?
        );
    } else {
        println!("aikit config (read-only):");
        println!("  root: {}", root.display());
        if cfg.sources.is_empty() {
            println!("  files: (none — every value is a built-in default)");
        } else {
            println!("  files (later wins): {}", cfg.sources.join(", "));
        }
        println!("  settings:");
        let width = SETTING_KEYS.iter().map(|k| k.len()).max().unwrap_or(0);
        for (key, value, source) in cfg.effective() {
            println!("    {key:<width$}  {value}  [{source}]");
        }
        println!(
            "note: the MCP server does NOT consult this configuration — it applies to the \
             CLI only."
        );
    }
    Ok(())
...
struct RawOutput {
    ...
    #[allow(dead_code)]
    _comment: Option<serde_json::Value>,
    /// Run directories to keep; `0` disables pruning.
