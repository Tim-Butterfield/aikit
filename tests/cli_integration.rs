...
fn local_workflow_end_to_end() {
    ...
    //
    // The script uses this platform's always-available runner rather than a POSIX shell:
    // a Windows host is not guaranteed one, and this walkthrough is meant to exercise the
    // whole flow everywhere rather than skip most of itself off Unix.
    fs::create_dir_all(p.join(".aikit/temp")).unwrap();
    let (rel, body) = if cfg!(windows) {
        (".aikit/temp/task.cmd", "@echo integration-task\r\n")
    } else {
        (".aikit/temp/task.sh", "#!/bin/sh\necho integration-task\n")
    };
    fs::write(p.join(rel), body).unwrap();

    let plan = aikit_json(p, &["script", "run", rel, "--print"]);
    assert_eq!(plan["kind"], "aikit.script_run");
    assert_eq!(
        plan["executed"], false,
        "--print must not execute the script"
    );
    // The interpreter path is platform-specific; what matters is that one was resolved and
    // that the script is the argv tail, never a concatenated shell string.
    assert!(
        plan["interpreter"].as_str().is_some_and(|s| !s.is_empty()),
        "a runner should have been resolved: {plan}"
    );
    let argv: Vec<&str> = plan["argv"]
        ...
        .collect();
    // argv records the *executed* path, which on Windows uses native separators because
    // cmd.exe rejects forward slashes. `script_path` keeps the portable spelling.
    let exec_rel = if cfg!(windows) {
        rel.replace('/', "\\")
    } else {
        rel.to_string()
    };
    assert_eq!(
        argv.last(),
        Some(&exec_rel.as_str()),
        "script is the last argv element"
    );
    assert_eq!(plan["script_path"], rel);
    assert_eq!(
        argv.first().copied(),
        plan["interpreter"].as_str(),
        "argv[0] is the resolved interpreter"
    );
    assert!(
