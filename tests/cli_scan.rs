...
//! exits 3 (blocked_secret_findings) under `--fail-on <severity>`.
...
#[test]
fn scan_secrets_fail_on_high_exits_three() {
    let repo = init_repo();
    fs::write(repo.path().join("c.txt"), format!("token = {FAKE_TOKEN}\n")).unwrap();
    let out = aikit(repo.path())
        .args(["scan", "secrets", "c.txt", "--fail-on", "high"])
        .assert()
        .failure()
        .code(3)
...
#[test]
fn scan_secrets_fail_on_high_clean_exits_zero() {
    let repo = init_repo();
    fs::write(repo.path().join("clean.txt"), "nothing to see here\n").unwrap();
    aikit(repo.path())
        .args(["scan", "secrets", "clean.txt", "--fail-on", "high"])
        .assert()
        .success();
}

#[test]
fn fail_on_high_ignores_medium_findings() {
    // The whole point of the severity threshold: a documentation-shaped `password = ...`
    // must not fail a build that only cares about self-identifying credentials. Gating on
    // any finding at all made these two cases indistinguishable.
    let repo = init_repo();
    fs::write(repo.path().join("notes.md"), "password = changeme\n").unwrap();

    let out = scan_json(repo.path(), &["notes.md"]);
    assert_eq!(out["counts"]["high"], 0);
    assert_eq!(out["counts"]["medium"], 1);

    aikit(repo.path())
        .args(["scan", "secrets", "notes.md", "--fail-on", "high"])
        .assert()
        .success();

    aikit(repo.path())
        .args(["scan", "secrets", "notes.md", "--fail-on", "medium"])
        .assert()
        .failure()
        .code(3);
}
...
#[test]
fn example_files_demote_the_name_based_rule_only() {
    // A placeholder in a template is `low`; a real credential format keeps `high` wherever
    // it lives, because a committed private key under fixtures/ is still a committed key.
    let repo = init_repo();
    fs::create_dir_all(repo.path().join("fixtures")).unwrap();
    fs::write(repo.path().join("config.example"), "password = changeme\n").unwrap();
    fs::write(
        repo.path().join("fixtures/key.pem"),
        "-----BEGIN OPENSSH PRIVATE KEY-----\n",
    )
    .unwrap();

    let out = scan_json(repo.path(), &["config.example"]);
    assert_eq!(out["counts"]["low"], 1, "placeholder in a template is low");
    assert_eq!(out["counts"]["medium"], 0);

    let out = scan_json(repo.path(), &["fixtures/key.pem"]);
    assert_eq!(
        out["counts"]["high"], 1,
        "a private key is high even under fixtures/"
    );
}

#[test]
fn self_identifying_formats_are_high() {
    // These are the rules that make `high` mean something, and therefore make
    // `--fail-on high` defensible as a CI gate.
    let repo = init_repo();
    let cases: &[(&str, &str)] = &[
        ("gh.txt", "ghp_abcdefghijklmnopqrstuvwxyz0123456789"),
        ("slack.txt", "xoxb-1234567890-abcdefghij"),
        ("stripe.txt", "sk_live_abcdefghijklmnop123456"),
        ("google.txt", "AIzaSyA1234567890abcdefghijklmnopqrstuv"),
        ("npm.txt", "npm_abcdefghijklmnopqrstuvwxyz0123456789"),
        (
            "jwt.txt",
    ...
    for (name, body) in cases {
        fs::write(repo.path().join(name), format!("{body}\n")).unwrap();
        let out = scan_json(repo.path(), &[name]);
        assert_eq!(
            out["counts"]["high"], 1,
            "{name} should produce exactly one high finding, got {out}"
        );
        // The value itself is never echoed back, whatever the rule.
        assert!(!serde_json::to_string(&out).unwrap().contains(body));
    }
}
