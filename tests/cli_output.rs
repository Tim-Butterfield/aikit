...
/// Set an old mtime (year 2000) so age filters treat the artifact as old.
///
/// This targets a *directory*, which is why it does not use `File::set_modified`: opening a
/// directory for writing fails on both platforms without platform-specific flags. `filetime`
/// handles that difference, and `touch` — the previous approach — does not exist on Windows.
fn set_old(path: &Path) {
    // 2000-01-01T00:00:00Z
    let when = filetime::FileTime::from_unix_time(946_684_800, 0);
    filetime::set_file_mtime(path, when)
        .unwrap_or_else(|e| panic!("set mtime for {}: {e}", path.display()));
}
