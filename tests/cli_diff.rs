//! Integration tests for `aikit batch diff`.
//!
//! `batch diff` is a mechanical, read-only Git diff from an anchor's recorded head to
...
#[test]
fn batch_help_lists_diff_and_the_top_level_diff_group_is_gone() {
    // `diff` was a top-level namespace holding one subcommand that operated on a batch
    // anchor, which promised a general diff facility aikit does not have. It now lives where
    // its subject does. A group that still parsed would be a second way to say the same thing.
...
fn diff_anchor_rejects_path_escape() {
    ...
    .unwrap();
    aikit(repo.path())
        .args(["batch", "diff", anchor.to_str().unwrap()])
        .assert()
        .failure()
        .code(3)
