...
// `>` is an illegal filename character on Windows, so the hazard this guards against —
// mistaking a filename for git porcelain's " -> " rename separator — cannot be constructed
// there at all. Gating is honest; contorting the name would test something else.
#[cfg(unix)]
#[test]
...
#[test]
fn batch_changed_rejects_an_anchor_path_outside_the_repo() {
    // An anchor path pointing into another repository is rejected as a path escape, the
    // same state `batch diff` reports for the same input: `batch changed` now resolves
    // anchors through the shared resolver instead of loading the path directly, so both
    // commands answer identically. The containment check runs before the anchor is parsed,
    // which is why this is `blocked_path_escape` rather than `blocked_invalid_anchor`.
    ...
    for command in [["batch", "changed", "--anchor"], ["batch", "diff", "--"]] {
        // `batch diff` takes the anchor positionally; `--` guards a path starting with `-`.
        let mut args: Vec<&str> = command.to_vec();
        args.push(foreign_anchor.to_str().unwrap());
        aikit(repo_b.path())
            .args(&args)
            .assert()
            .failure()
...
#[test]
fn batch_changed_rejects_an_in_repo_anchor_belonging_to_another_repo() {
    // The containment check cannot catch this one: the file sits inside this repository, so
    // only parsing it reveals that its recorded repo_root is somewhere else. Keeping this
    // separate from the path-escape case ensures consolidating the resolver did not quietly
    // drop the ownership validation.
    let repo_a = init_repo();
    aikit(repo_a.path())
        .args(["batch", "start"])
        .assert()
    ...
    let repo_b = init_repo();
    let batches = repo_b.path().join(".aikit/outputs/batches");
    fs::create_dir_all(&batches).unwrap();
    let planted = batches.join(foreign_anchor.file_name().unwrap());
    fs::copy(&foreign_anchor, &planted).unwrap();

    // Referenced by id, the way `batch start` prints it.
    let id = planted.file_stem().unwrap().to_str().unwrap().to_string();
    aikit(repo_b.path())
        .args(["batch", "changed", "--anchor", &id])
        .assert()
        .failure()
        .code(3)
